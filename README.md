...
| Quick Add | 支持中文日期、紧凑时间、repeat 前缀和 `MMDDHHMM title`。 |
...
2. 点击右上角 `+`。
3. 输入普通任务，或输入 `明天0930 复习 JVM` 这类 Quick Add。
...
Quick Add 可以在一行里写出时间、重复规则和任务内容。
...
| 输入 | 解析 |
| --- | --- |
| `明天0930 复习 JVM` | due date = 明天，reminder = 09:30，text = 复习 JVM |
| `后天0600 起床` | due date = 后天，reminder = 06:00 |
| `每天2100 背单词` | repeat = daily，reminder = 21:00 |
| `周五1120 复习JUC` | due date = 下一个周五，reminder = 11:20 |
