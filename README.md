...
<p align="center">
  Chrome 工具栏里的待办 / 打卡 / 提醒插件。<br>
  记一条小事，到点提醒你，点掉就完；重复的任务会自动再出现。
</p>
...
Sisyphus 用来记那些不值得开项目管理工具、又怕忘的小事，比如 `明天0930 干饭`、`每天2100 干饭`、`周五1120 干饭`。点扩展图标弹出 popup；关掉之后，提醒和重复任务还是由 Chrome alarms 在后台排程。

没有账号、没有云端、没有 dashboard，数据都存在本地。

## ✨ 核心三件事

🧠 **一句话拆成字段**<br>
Quick Add 从一整句里认出日期、时间、repeat，剩下的文字就是任务名。

🔔 **popup 没开也能提醒**<br>
后台 alarm 继续排程，Chrome 桌面通知给你 `Snooze` 和 `Done`，不打开窗口也能标记完成。

🔁 **重复任务自动重置**<br>
daily / weekly / monthly 完成后，到下一周期会变回未完成。
